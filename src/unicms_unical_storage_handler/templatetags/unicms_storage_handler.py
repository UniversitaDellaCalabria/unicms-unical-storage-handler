...
ALLOWED_UNICMS_SITES = getattr(
    settings, "ALLOWED_UNICMS_SITES", app_settings.ALLOWED_UNICMS_SITES
)
CMS_STORAGE_BASE_API = getattr(
    settings, "CMS_STORAGE_BASE_API", app_settings.CMS_STORAGE_BASE_API
)
CMS_STORAGE_CDS_WEBSITE_PROSPECT_DOCS = getattr(
    settings,
...
def get_allowed_website(host):
    ...
    if "*" in ALLOWED_UNICMS_SITES:
        ...
        websites = WebSite.objects.filter(pk__in=ALLOWED_UNICMS_SITES, is_active=True)
...
@register.simple_tag
def pe_api_root(value):
    app_root = getattr(app_settings, "CMS_PE_BASE_API", "")
    settings_root = getattr(settings, "CMS_PE_BASE_API", app_root)
...
def get_cds_website_root_path(cds_cod=None):
    ...
    webpath_cds = WebPathCdsCod.objects.filter(
        webpath__is_active=True, cds_cod=cds_cod
    ).first()
    if not webpath_cds:
        return None
...
def get_cds_website_morph(cds_cod):
    ...
    cds_morph_list = requests.get(
        f"{CMS_STORAGE_BASE_API}{cds_morph_api.format(cds_cod)}"
    )
    return cds_morph_list.json()
